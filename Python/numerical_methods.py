...
def incremental_search(func, xl: float, xu: float = None,
                       dx: float = 0.1, tol: float = 1e-6):
    """
    Scan from xl in steps of dx, locating sign-change brackets.
    If xu is provided, stop at xu; otherwise keep moving rightward
    until a sign change is found or a safety limit is reached.
    ...
    if abs(f_cur) < tol:
        return x_cur, [{"Iteration": 1, "XL": _fmt(x_cur), "Dx": _fmt(dx),
                        "XU": _fmt(x_cur), "f(XL)": _fmt(f_cur),
                        "f(XU)": _fmt(f_cur), "f(XL).f(XU)": "= 0",
                        "Remark": "Exact root found"}]

    while it <= max_steps:
        x_next = x_cur + dx
        if xu is not None and x_next > xu:
            x_next = xu
        ...
        if abs(prod) < 1e-14 or prod < 0 or abs(f_next) < tol:
            break
...
def bisection(func, xl: float, xu: float,
    ...
    for i in range(1, max_iter + 1):
        ...
        # Convergence checks
        if abs(xu - xl) < tol or abs(f_xr) < tol:
            remark = "Converged ✔"
        if abs(f_xr) < tol:
            remark = "Exact root found"
