...
class NumericalTab(tk.Frame):
    ...
    def _dispatch(self, m, a, b, tol, mi, dx=None):
        ...
        if m == "Incremental Search":
            step = dx if dx is not None else abs(b - a) / 100.0
            return incremental_search(f, a, b, dx=step, tol=tol)
        if m == "Bisection":
...
class MatrixTab(tk.Frame):
    ...
    def _on_op_change(self, _=None):
        ...
        if need_b:
            ...
            # Clear Matrix B input when operation changes
            self.tb.delete("1.0", "end")
        else:
            self._sec_b.grid_remove()
            # Ensure Matrix B is empty when not needed
            self.tb.delete("1.0", "end")
        ...
        # Clear Matrix A as well and the result output when operation changes
...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Numerical Methods & Matrix Operations  v6")
    ...
    def _build_ui(self):
        ...
        tk.Label(
            hdr,
            text="  🔢  Numerical Methods & Matrix Operations",
            bg=HDR_BG, fg="white",
